...
if(!message.member.hasPermission("KICK_MEMBERS")) return message.channel.send(`Bu komutu kullanman için \`Üye Yasakla\` Yetkisine sahip olan gerek!`);
  ...
  if (reason.length < 1) return message.channel.send('Örnek kullanım: `xy!uyar @Kullanıcı Kural İhlali`');
  if (message.mentions.users.size < 1) return message.channel.send('Örnek kullanım: `xy!uyar @Efe Kural İhlali`')
